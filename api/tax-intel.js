...
// ── Manual intel override ─────────────────────────────────────────────────────
// If intel.json exists in the repo root, it is used instead of calling the API.
// Workflow: generate brief in Claude chat → paste JSON → save as intel.json → push to GitHub.
const fs   = require("fs");
const path = require("path");

function loadManualIntel() {
  try {
    const file = path.join(process.cwd(), "intel.json");
...
module.exports = async function handler(req, res) {
  ...
  // 1. Cache hit: same week, not forced
  ...
  // 2. Manual intel.json override (no API key needed)
  const manualIntel = loadManualIntel();
  const news        = manualIntel || [];
  const intelSource = manualIntel ? "intel.json" : null;
  let   intelError  = null;

  // 3. Fall back to API only if no manual file and key is available
  let apiNews = [];
  if (!manualIntel) {
    const result = await generateIntelligence(apiKey);
    apiNews      = result.items;
    intelError   = result.error;
  }

  const finalNews = manualIntel ? news : apiNews;
  ...
  const body = {
    ...
    meta: {
      ...
      cached:       false,
      cache_age_s:  0,
      news_count:   finalNews.length,
      source:       intelSource || "claude-sonnet-4-20250514+web_search",
