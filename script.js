...
function updateMeta() {
  ...
  const nextRefresh = m.next_refresh
    ? `Next: ${new Date(m.next_refresh).toLocaleDateString("en-GB", { weekday:"short", day:"numeric", month:"short" })}`
    : null;

  el.metaBar.textContent = [
    m.week           ? `Brief: ${m.week}` : null,
    m.fetched_at     ? `Generated ${timeAgo(m.fetched_at)}` : null,
    m.cached         ? `Cached (+${m.cache_age_s}s)` : null,
    m.news_count     != null ? `${m.news_count} items` : null,
    nextRefresh,
    m.intel_error    ? `⚠ ${m.intel_error}` : null,
  ].filter(Boolean).join(" · ");
}
...
function checkStaleness() {
  ...
  if (ageH >= 24) {
    ...
    el.staleBanner.innerHTML =
      ...
      `<button class="banner-retry" data-action="regen">Regenerate now</button>`;
    ...
    items.forEach(item => {
      ...
      html += `
        <div class="intel-card ${esc(item.priority)}" data-expandable>
          ...
          ${item.razer_relevance ? `<div class="intel-relevance">↳ ${esc(item.razer_relevance)}</div>` : ""}
...
function setupDelegated() {
  ...
  el.content.addEventListener("click", e => {
    ...
    if (act?.dataset.action === "retry")   { fetchData();        return; }
    if (act?.dataset.action === "regen")   { fetchData(true);    return; }
  });
...
function buildToolbar() {
  ...
  // Regenerate intel (force refresh, bypasses cache)
  $("regen-btn")?.addEventListener("click", () => fetchData(true));
}
